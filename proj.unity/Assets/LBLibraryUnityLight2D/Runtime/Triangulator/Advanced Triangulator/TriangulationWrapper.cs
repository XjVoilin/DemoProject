...
namespace LIBII.Light2D
{
    public static class TriangulationWrapper
    {
        ...
        public static Mesh CreateMesh(Polygon polygon, bool keepHoleTriangles = false)
        {
            ...
            LIBII.Light2D.Polygon poly =
                new LIBII.Light2D.Polygon(ConvertPoints(polygon.outside, codeToPosition));

            foreach (List<Vector2> hole in polygon.holes)
                poly.AddHole(new LIBII.Light2D.Polygon(ConvertPoints(hole, codeToPosition)));
            ...
            if (keepHoleTriangles)
            {
                foreach (var polyHole in poly.Holes)
                {
                    triangles.AddRange(polyHole.Triangles);
                }
            }

            foreach (DelaunayTriangle t in triangles)
            {

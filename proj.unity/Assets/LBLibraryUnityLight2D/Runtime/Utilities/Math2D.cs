...
namespace LIBII.Light2D
{
    static public class Math2D
    {
        ...
        */
        ...
        {
            float roll = Mathf.Atan2(2 * rotation.y * rotation.w - 2 * rotation.x * rotation.z,
                1 - 2 * rotation.y * rotation.y - 2 * rotation.z * rotation.z);
            float pitch = Mathf.Atan2(2 * rotation.x * rotation.w - 2 * rotation.y * rotation.z,
        ...
        public static Rect GetBounds(List<Vector2D> pointsList)
        {
            ...
            return (new Rect((float) rMinX, (float) rMinY, (float) System.Math.Abs(rMinX - rMaxX),
                (float) System.Math.Abs(rMinY - rMaxY)));
        }
        ...
        static public bool LineIntersectCircle(Pair2D line, Vector2D circle, float radius)
        {
            ...
            double q = ((circle.x - line.A.x) * (line.B.x - line.A.x) + (circle.y - line.A.y) * (line.B.y - line.A.y)) /
                       (sx * sx + sy * sy);

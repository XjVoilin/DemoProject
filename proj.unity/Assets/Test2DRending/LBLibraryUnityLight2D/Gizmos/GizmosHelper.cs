...
namespace LIBII.Light2D
{
    public static class GizmosHelper
    {
        static public void DrawRect(Vector3 position, Rect rect)
        {
            ...
            Vector3 p2 =
                LightingPosition.GetPosition3DWorld(new Vector2(rect.x + rect.width, rect.y + rect.height), position);
        ...
        static public void DrawRect(Rect worldRect)
        {
            Vector3 p0 = worldRect.min;
            Vector3 p1 = new Vector2(worldRect.max.x, worldRect.min.y);
            Vector3 p2 = worldRect.max;
            Vector3 p3 = new Vector2(worldRect.min.x, worldRect.max.y);
        ...
        static public void DrawCircle(Vector3 position, float rotation, float angle, float size)
        {
            ...
            for (int i = start; i < end; i += step)
            {
                ...
                Vector3 pointA =
                    LightingPosition.GetPosition3D(new Vector2(Mathf.Cos(rotA) * size, Mathf.Sin(rotA) * size), center);
                Vector3 pointB =
                    LightingPosition.GetPosition3D(new Vector2(Mathf.Cos(rotB) * size, Mathf.Sin(rotB) * size), center);
